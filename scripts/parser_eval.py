...
def RewriteContext(task_context):
  ...
  for resource2 in context.input:
    if resource2.name == FLAGS.output:
      for part2 in resource2.part:
        out_pat = part2.file_pattern

  contexts = []
  for resource in context.input:
    if resource.name == FLAGS.input:
      for part in resource.part:
        if part.file_pattern != '-':
          for name in glob.glob(part.file_pattern):
            part.file_pattern = name
            for resource2 in context.input:
              if resource2.name == FLAGS.output:
                for part2 in resource2.part:
                  part2.file_pattern = os.path.splitext(name)[0]+'.out'+out_pat
            with tempfile.NamedTemporaryFile(delete=False) as fout:
              fout.write(str(context))
              contexts.append(fout.name)
...
def run_parser(sess, parser, sink, sink_documents):
  t = time.time()
  num_epochs = None
  num_tokens = 0
  num_correct = 0
